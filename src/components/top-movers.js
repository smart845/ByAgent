...
export function spawnTopMovers(){
  ...
  function render(){
    ...
    container.innerHTML = `<div id="moversWrap">${merged.map(rowHTML).join('')}</div>`;

    container.querySelectorAll('.coin-row .sym').forEach(el=>{
      el.addEventListener('click', ()=> {
        const sym = el.parentElement.dataset.sym;
        document.dispatchEvent(new CustomEvent('ui:selectSymbol', { detail:{ symbol:sym } }));
      });
    });
    container.querySelectorAll('.coin-row .fav').forEach(btn=>{
    ...
    container.querySelectorAll('.coin-row .add-alert').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        const sym = btn.dataset.sym;
